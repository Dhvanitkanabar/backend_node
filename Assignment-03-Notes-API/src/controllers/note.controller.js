...
const createNote = async (req, res) => {
  try {
    ...
    const createdNote = await Note.create({ title, content, category, isPinned });
...
const createBulkNotes = async (req, res) => {
  try {
    ...
    const createdNotes = await Note.insertMany(notes, { runValidators: true });
...
const masterQuery = async (req, res) => {
  try {
    const { q, category, isPinned, sortBy = "createdAt", order = "desc" } = req.query;
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const skip = (page - 1) * limit;
